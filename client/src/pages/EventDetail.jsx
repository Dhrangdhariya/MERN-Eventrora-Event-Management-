...
const EventDetail = () => {
    ...
    // 1. Helper function to dynamically mount Razorpay script from their official CDN
    ...
    // 2. Streamlined Booking and Payment execution handler
    const handleBookingAndPaymentFlow = async (e) => {
        ...
        try {
            if (!showOTP) {
                // First Phase: Request OTP token generation dispatch
                await api.post('/bookings/send-otp');
                setShowOTP(true);
                setSuccessMsg('OTP verified validation codes dispatched.');
                setBookingLoading(false);
                return;
            }

            // Second Phase (OTP entered): Initialize Razorpay order mapping handshake
            const scriptLoaded = await loadRazorpayScript();
            if (!scriptLoaded) {
                alert('Razorpay Payment Gateway failed to initialize. Check connection.');
                setBookingLoading(false);
                return;
            }

            const orderRequest = await api.post('/bookings/initialize-order', {
                eventId: event._id,
                otp
            });

            const { order_id, amount, currency, key_id } = orderRequest.data;

            // Build structural production/sandbox options configurations
            const paymentOptions = {
                key: key_id,
                amount: amount,
                currency: currency,
                name: "Eventora Platform",
                description: `Pass Reservation for ${event.title}`,
                order_id: order_id,
                handler: async function (response) {
                    try {
                        ...
                        // Send secure signature mapping straight to back-end to finalize seats
                        await api.post('/bookings', finalBookingPayload);
                        alert('🎉 Payment Captured Safely! Your ticket is confirmed.');
                        navigate('/dashboard');
                    } catch (err) {
                        alert(err.response?.data?.message || 'Verification database sync failure.');
                    } finally {
                        setBookingLoading(false);
                    }
                },
                prefill: {
                    name: user?.name || "Attendee Account",
                    email: user?.email || "test@example.com",
                },
                theme: {
                    color: "#111827", // Accent Match: Charcoal Minimalist
                }
            };

            const razorpayOverlayWindow = new window.Razorpay(paymentOptions);
            razorpayOverlayWindow.open();

        } catch (err) {
            setError(err.response?.data?.message || 'Transaction initialization configuration broken');
        } finally {
            setBookingLoading(false);
        }
    };
            ...
            <div className="p-8 md:p-12">
                <div className="flex flex-col md:flex-row justify-between items-start mb-8 gap-6">
                    ...
                    <div className="bg-gray-50 p-6 rounded-xl border border-gray-100 min-w-[300px] w-full md:w-auto shrink-0 shadow-sm">
                        ...
                        <button
                            onClick={handleBookingAndPaymentFlow}
                        ...
                        >
                            {bookingLoading ? 'Processing...' : (showOTP ? 'Pay & Secure Ticket' : 'Confirm Registration')}
                        </button>
